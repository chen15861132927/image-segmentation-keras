...
def find_latest_checkpoint(checkpoints_path, fail_safe=True):
    ...
    def check_all_number_from_path(path):
        splits= path.replace(checkpoints_path, "").strip(".").split(".")
        isallnumber=True
        for item in splits:
    ...
    filterall_checkpoint_files = list(filter(lambda f: check_all_number_from_path(f), all_checkpoint_files))
    if not len(filterall_checkpoint_files):
    ...
    latest_epoch_checkpoint = max(filterall_checkpoint_files,
                                  key=lambda f:
...
def train(model,
    ...
    if auto_resume_checkpoint and (checkpoints_path is not None):
        ...
        if latest_checkpoint is not None:
            ...
            initial_epoch = int(get_epoch_number_from_path(latest_checkpoint))
    ...
    if callbacks is None and (not checkpoints_path is  None) :
        default_callback = ModelCheckpoint(
                filepath=checkpoints_path + ".{epoch:05d}.{accuracy:.06f}.{val_accuracy:.06f}",
