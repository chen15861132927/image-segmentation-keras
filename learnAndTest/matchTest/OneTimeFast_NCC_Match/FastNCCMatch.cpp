...
void FastNCCMatch::LearnPattern(Mat &templateImage)
{
	...
	int vecPyramidCount = templData->vecPyramid.size();
	templData->resize(vecPyramidCount);

	for (int i = 0; i < vecPyramidCount; i++)
	{
		Mat currentPyramid = templData->vecPyramid[i];

		double invArea = 1. / ((double)currentPyramid.rows * currentPyramid.cols);
		...
		double templSum2 = templNorm + templMean.at<double>(0, 0)*templMean.at<double>(0, 0); ;// templMean[0] * templMean[0] + templMean[1] * templMean[1] + templMean[2] * templMean[2] + templMean[3] * templMean[3];
		...
		templData->vecTemplMean[i] = templMean.at<double>(0, 0);
...
void FastNCCMatch::CCOEFF_Denominator(cv::Mat& matSrc, cv::Mat &currentTemplete, cv::Mat& matResult)
{
	Mat sum, sumOfSquare;
	integral(matSrc, sum, sumOfSquare, CV_64F);
	Mat templMean, templSdv;//Scalar templMean, templSdv;
	meanStdDev(currentTemplete, templMean, templSdv);
	double area = ((double)currentTemplete.rows * currentTemplete.cols);
	double denominatorPart_Templete = templSdv.at<double>(0, 0)*std::sqrt(area);
	...
	for (int i = 0; i < matResult.rows; i++)
	{
		for (int j = 0; j < matResult.cols; j++)
		{
			...
			double q0data = sumOfSquare.at<double>(i, j);
			double q1data = sumOfSquare.at<double>(i, j + currentTemplete.cols);
			double q2data = sumOfSquare.at<double>(i + currentTemplete.rows, j);
			double q3data = sumOfSquare.at<double>(i + currentTemplete.rows, j + currentTemplete.cols);

			double sumFxy = p0data - p1data - p2data + p3data;
			double numerator = matResult.at<float>(i, j) - templMean.at<double>(0, 0)*sumFxy;

			double srcSquare = q0data - q1data - q2data + q3data;
			double denominatorPart_F = srcSquare - (sumFxy * sumFxy / area);

			denominatorPart_F = MAX(denominatorPart_F, 0);
			double denominator = 0;
			if (denominatorPart_F <= min(0.5, 10 * FLT_EPSILON * srcSquare))
				denominator = 0; // avoid rounding errors
			else
				denominator = std::sqrt(denominatorPart_F)*denominatorPart_Templete;
...
//#define ORG
void  FastNCCMatch::MatchTemplate(cv::Mat& matSrc, cv::Mat &matTemplate, cv::Mat& matResult)
{
	...
	CCOEFF_Denominator(matSrc, matTemplate, matResult);
...
float FastNCCMatch::Match(Mat &searchImage, vector<s_SingleTargetMatch> &outFind)
{
	...
	for (int i = 0; i < topSeachAngleSize; i++)
	{
		...
		cv::Mat matTemplate = pTemplData->vecPyramid[iTopLayer];
		//MatchTemplate(matRotatedSrc, matTemplate, pTemplData, matResult, iTopLayer, true);
		...
		else
		{
			for (int iLayer = iTopLayer - 1; iLayer >= iStopLayer; iLayer--)
			{
				...
				for (int j = 0; j < currentLayerAngleSize; j++)
				{
					...
					GetRotatedROI(MatSrcPyr, pTemplData->vecPyramid[iLayer].size(), ptLT * 2, vecAngles[j], matRotatedSrc);
					cv::Mat matTemplate = pTemplData->vecPyramid[iLayer];
					//MatchTemplate(matRotatedSrc, matTemplate, pTemplData, matResult, iLayer, true);
