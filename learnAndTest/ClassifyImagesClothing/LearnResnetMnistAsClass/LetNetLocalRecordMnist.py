...
def get_dataset(filenames):
    ...
    dataset = dataset.shuffle(204800)
    dataset = dataset.prefetch(buffer_size=tf.data.AUTOTUNE)
    dataset = dataset.batch(51200)
    return dataset
batch_size=128

train_dataset = get_dataset("MnisttrainTotal.tfrecords")
...
ResnetTrain.trainWithOutTest(x_train=np.copy(x_train), y_train=y_train,epochs=epochs,batch_size=batch_size,Nodata_augmentation=True,
