...
# 训练参数
batch_size = 100  # 原论文按照 batch_size=128 训练所有的网络
...
#第三步：对需要处理的数据进行fit
datagenx_train=x_test#x_train
datagen.fit(datagenx_train)


#第四步：使用.flow方法构造Iterator， 
data_iter = datagen.flow(datagenx_train, y_test, batch_size=batch_size)  #返回的是一个“生成器对象”
print(type(data_iter))    #返回的是一个NumpyArrayIterator对象
 

epochsIndex=0
totalcount=0
path=os.path.join(Root_Path,"MnistDatagenTest")
