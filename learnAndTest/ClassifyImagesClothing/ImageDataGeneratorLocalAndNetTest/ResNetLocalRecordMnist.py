...
def get_dataset(filenames,shuffle,totalcount):
    ...
    return dataset
batch_size=256
trainRecordFile = os.path.join(Root_Path,"MnisttrainTotal.tfrecords")
#testRecordFile = os.path.join(Root_Path,"MnisttestTotal.tfrecords")
#train_dataset = get_dataset(trainRecordFile,900000,512000)
train_dataset = get_dataset(trainRecordFile,9000,1000)

#test_dataset = get_dataset(testRecordFile,120000,80000)

x_train, y_train = next(iter(train_dataset))
#x_test, y_test = next(iter(test_dataset))
(x_origintrain, y_origintrain), (x_origintest, y_origintest) = tf.keras.datasets.mnist.load_data()
x_origintest = np.expand_dims(x_origintest, -1)

ResnetTrain= Resnet(version=2,n=4,subtract_pixel_mean=True)
input_shape=(28, 28, 1)
epochs = 50
num_classes = 10
...
# 数据标准化。
x_train = np.copy(x_train).astype('float32') / 255
#x_test = np.copy(x_test).astype('float32') / 255
x_test = np.copy(x_origintest).astype('float32') / 255
...
# 将类向量转换为二进制类矩阵。
y_train = tf.keras.utils.to_categorical(y_train.numpy(), num_classes)
#y_test = tf.keras.utils.to_categorical(y_test.numpy(), num_classes)
y_test = tf.keras.utils.to_categorical(y_origintest, num_classes)
ResnetTrain.Root_Path=Root_Path
ResnetTrain.train(x_train=x_train, y_train=y_train,x_test=x_test,
 y_test=y_test,epochs=epochs,batch_size=batch_size,Nodata_augmentation=True,
 modelFile='MnistResNet38v2_modelcp-0128-0.9953.h5')
