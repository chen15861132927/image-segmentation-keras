...
class Resnet:
    ...
    def compile(self,input_shape):
        ...
        self.model.compile(loss='categorical_crossentropy',
                    ...
                    metrics=['acc'])
        # self.model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=self.lr_schedule(0)),
    ...
    def train(self,x_train, y_train,x_test, y_test,epochs=200,batch_size=32,Nodata_augmentation=True,modelFile=""):
        ...
        # 准备模型保存路径。
        self.save_dir = os.path.join(self.Root_Path, 'LearnResNetMnistSaved_models'+ self.model_type)
        # 如果使用减去像素均值
        if self.subtract_pixel_mean:
            self.x_train_mean = np.mean(x_train, axis=0)
            x_train -= self.x_train_mean
            x_test -= self.x_train_mean
        model_name = 'Mnist%s_model-{epoch:03d}-{acc:.03f}-{val_acc:.04f}.h5' % self.model_type
        if not os.path.isdir(self.save_dir ):
            os.makedirs(self.save_dir )
        filepath = os.path.join(self.save_dir , model_name)
        print(filepath)

        loadfile =  os.path.join(self.save_dir , modelFile)
        if os.path.isfile(loadfile):
            self.model = tf.keras.models.load_model(loadfile)
            print("readed model",loadfile)
